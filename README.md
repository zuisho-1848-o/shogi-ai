...
- **Web UI**: ブラウザで人間 vs AI 対局・AI vs AI オートプレイ・リアルタイム評価値グラフ・MultiPV候補手表示
- **OSSエンジン統合**: YaneuraOu などの USI エンジンをUIから選択可能
...
| エンジン | 評価関数 | 探索 | 特徴 |
|---------|---------|------|------|
| PST + αβ | 駒得 (piece-square table) | Alpha-Beta | 標準・軽量 |
| KPP + αβ | KP評価（機械学習済み） | Alpha-Beta | 中程度 |
| NNUE + αβ | ニューラルネット | Alpha-Beta | 強め |
| PST + MCTS | 駒得 | モンテカルロ木探索 | 実験的 |

### 外部USIエンジン（YaneuraOu など）

外部USIエンジンは `engines/` 配下にローカル配置して使う。バイナリや評価ファイルは環境差分・大容量化が起きやすいため `.gitignore` 対象。

配置例:

| パス | 用途 |
|---|---|
| `engines/yaneuraou/YaneuraOu-Material-ARM64` | 軽量な駒得評価版 |
| `engines/yaneuraou/YaneuraOu_KPPT-V900Git_APPLEAVX2` | KPPT評価版 |

UIのプレイヤー設定で「USIエンジン」を選び、パスを入力すると使用できる:
```
engines/yaneuraou/YaneuraOu-Material-ARM64
```

より強いエンジンにしたい場合は YaneuraOu の NNUE 版をビルドし、評価ファイル（`nn.bin` / `*.nnue` など）を同じエンジン配下に配置する。
...
- 先手・後手ごとに **人間 / 内蔵エンジン / USIエンジン** を選択可能
- **AI vs AI オートプレイ** で内蔵エンジン同士・外部USIエンジンとの対戦を観戦可能

内蔵エンジンはUIから次を選べる:

| ID | 内容 |
|---|---|
| `pst_alphabeta` | PST評価 + Alpha-Beta探索（標準） |
| `kpp_alphabeta` | KPP評価 + Alpha-Beta探索（`models/kpp.npz` がなければ PST にフォールバック） |
| `nnue_alphabeta` | NNUE評価 + Alpha-Beta探索（`models/nnue.npz` がなければ PST にフォールバック） |
| `pst_mcts` | PST評価 + MCTS |
...
| [web/usi_engine.py](web/usi_engine.py) | 外部USIエンジンの非同期サブプロセスラッパー |
